...
export const Route = createFileRoute("/about")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "About Hotflame Biogas, a clean energy startup transforming organic waste into sustainable biogas solutions.",
      },
    ],
  }),
  component: About,
    ...
    <>
      <section className="bg-white">
        <div className="mx-auto max-w-7xl px-5 py-20 md:px-8 lg:flex lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-700">About Hot Flame Biogas</p>
            ...
            <p className="mt-6 text-lg leading-8 text-zinc-600">
              We are a clean energy enterprise based in Narok Town, Kenya. We design and install biogas systems that improve health, lower costs, and protect the environment.
            </p>
          </div>
          <div className="mt-10 lg:mt-0 lg:w-[520px]">
            <div className="overflow-hidden rounded-[2rem] border border-zinc-200 shadow-lg">
              <img src="/assets/biogas plant.jpeg" alt="Biogas plant in the field" className="h-[440px] w-full object-cover" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-14">
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-5 md:px-8 lg:grid-cols-[0.95fr_1fr]">
          <div className="relative">
            <img src="/assets/650836646_122161105478958675_3445836754463755117_n.jpg" alt="Mary Mwami, Founder and CEO" className="h-[470px] w-full rounded-xl object-cover object-top shadow-xl" />
            <div className="absolute bottom-8 right-[-10px] hidden max-w-48 rounded-lg bg-white p-3 shadow-xl md:block">
              <img src="/assets/645415092_122159713724958675_2122180214002289931_n.jpg" alt="Biogas dome" className="h-24 w-full rounded object-cover" />
              <p className="mt-2 text-xs font-black text-zinc-700">Building sustainable solutions that power a cleaner tomorrow.</p>
            </div>
          </div>
          <div>
            <p className="text-sm font-black uppercase tracking-widest text-emerald-700">Our Story</p>
            <h2 className="mt-3 text-3xl font-black leading-tight text-zinc-950 md:text-4xl">From a Vision to a Movement</h2>
            ...
            <p className="mt-1 text-sm font-bold text-zinc-950">Mary Mwami, <span className="font-medium text-zinc-600">Founder & CEO</span></p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white pb-12">
        <div className="mx-auto grid max-w-7xl gap-6 px-5 md:grid-cols-3 md:px-8">
          {[
            { icon: Target, title: "Our Mission", text: "To provide innovative biogas solutions that convert organic waste into clean energy, empowering communities and protecting the planet." },
            { icon: Eye, title: "Our Vision", text: "A world where every home and institution has access to affordable, renewable energy for a healthier, sustainable future." },
            { icon: Heart, title: "Our Core Values", text: "Sustainability, Integrity, Innovation, Quality, and Community. These values guide everything we do." },
          ].map((item) => (
            <article key={item.title} className="rounded-xl border border-zinc-200 bg-white p-7 shadow-lg">
      ...
      <section className="relative overflow-hidden bg-emerald-950 py-12 text-white">
        ...
        <div className="relative mx-auto max-w-7xl rounded-xl border border-white/25 bg-white/10 px-5 py-8 backdrop-blur md:px-8">
          <p className="mb-8 text-center text-sm font-black uppercase tracking-[0.3em]">Impact at a Glance</p>
          <div className="grid grid-cols-2 gap-6 text-center md:grid-cols-4">
            {[
              ...
              <div key={item.label} className="md:border-r md:border-white/25 last:border-r-0">
                ...
                <p className="mx-auto mt-1 max-w-28 text-xs font-bold text-white/85">{item.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-14">
        <div className="mx-auto grid max-w-7xl gap-10 px-5 md:px-8 lg:grid-cols-[0.95fr_1.05fr]">
          <div>
            <p className="text-sm font-black uppercase tracking-widest text-emerald-700">Why Choose Us?</p>
            <h2 className="mt-2 text-3xl font-black leading-tight text-zinc-950 md:text-4xl">Quality. Reliability. Sustainability.</h2>
            ...
            <img src="/assets/645415092_122159713724958675_2122180214002289931_n.jpg" alt="Completed biogas digester" className="mt-7 h-44 w-full max-w-md rounded-lg object-cover shadow-lg" />
          </div>
          <div>
            <p className="text-sm font-black uppercase tracking-widest text-emerald-700">Our Journey</p>
      ...
      <section className="bg-cover bg-center py-14" style={{ backgroundImage: "linear-gradient(rgba(255,255,255,.82), rgba(255,255,255,.82)), url('/assets/terrain.jpeg')" }}>
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <p className="text-sm font-black uppercase tracking-widest text-emerald-700">Media & Community Outreach</p>
          <h2 className="mt-2 text-3xl font-black text-zinc-950">Sharing Knowledge. Building Communities.</h2>
          <p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-zinc-700">
            We engage, educate, and collaborate with communities through media, trainings, and partnerships to promote clean energy solutions.
          </p>
          <div className="mt-8 grid gap-4 md:grid-cols-4">
            {media.map((image, index) => (
              <img key={image} src={image} alt={`Media outreach ${index + 1}`} className="h-44 w-full rounded-lg object-cover shadow-lg" />
            ))}
          </div>
      ...
      <section className="bg-emerald-950 px-5 py-8 text-white md:px-8">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-5 rounded-xl border border-white/30 bg-emerald-900 px-6 py-7">
          <div>
            <h2 className="text-2xl font-black md:text-3xl">Be Part of the Clean Energy Movement</h2>
            <p className="mt-1 text-sm font-medium text-white/85">Let's build a sustainable future, together.</p>
          </div>
          <a href="https://wa.me/254792934102" className="inline-flex items-center gap-2 rounded-md border border-white bg-emerald-700 px-8 py-4 text-sm font-black text-white transition hover:bg-white hover:text-emerald-900">
